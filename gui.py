...
class SCMonitorApp:
    def __init__(self, root, config):
        ...
        self._dpsk_validation_queue = queue.SimpleQueue()
    ...
    def _show_dpsk_token_warning(self, reason):
    ...
    def _start_dpsk_token_validation(self):
        if self._dpsk_client is None or self._dpsk_executor is None:
            return
        future = self._dpsk_executor.submit(self._dpsk_client.validate_token)
        future.add_done_callback(self._queue_dpsk_validation_result)

    def _queue_dpsk_validation_result(self, future: Future):
        try:
            ...
            is_valid = None
            error = str(exc)
        self._dpsk_validation_queue.put((is_valid, error))

    def _poll_dpsk_validation_results(self):
        while True:
            try:
                is_valid, error = self._dpsk_validation_queue.get_nowait()
            except queue.Empty:
                break

            if is_valid is False:
                logger.warning("DPSK_API_TOKEN 无效，已禁用 Steam ID 分析")
                self._dpsk_client = None
                records_updated = False
                for record in getattr(self, "_sc_records", []):
                    if record.get("steam_analysis_status") == "pending":
                        record["steam_analysis_status"] = "disabled"
                        records_updated = True
                if records_updated:
                    self._refresh_sc_list()
                self._show_dpsk_token_warning(
                    "DPSK_API_TOKEN 无效（DeepSeek 返回 401 认证失败），"
                    "Steam ID 自动分析将不可用。"
                )
            elif error:
                logger.warning("启动时无法验证 DPSK_API_TOKEN: %s", error)
            else:
                logger.info("DPSK_API_TOKEN 验证成功")
    ...
    def _poll_dpsk_results(self):
        ...
        self._poll_dpsk_validation_results()
